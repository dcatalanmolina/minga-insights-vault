...
| `decision-guide` agent / `frame-decision` skill | Keeney, R. (1992), *Value-Focused Thinking: A Path to Creative Decisionmaking* | Starting from fundamental objectives (what you value) to derive alternatives, rather than starting from a preset list of options. |
| `decision-guide` agent / `compare-options` skill | Kepner, C. & Tregoe, B., *The Rational Manager* (1965; rev. eds. since) | Separating MUST criteria (disqualifying) from weighted WANT criteria to score alternatives and surface adverse consequences. |
| `decision-guide` agent / `stress-test-decision` skill | Klein, G. (2007), ["Performing a Project Premortem"](https://hbr.org/2007/09/performing-a-project-premortem), *Harvard Business Review* | Imagining a decision has already failed and working backward to surface risks before committing. |
